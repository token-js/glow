...
from typing import Any, AsyncIterable, Awaitable, Callable, Dict, List, Literal, Optional, Union
import uuid

from livekit import rtc

from livekit.agents import stt, tokenize, tts, utils, vad, AgentState, ATTRIBUTE_AGENT_STATE
from livekit.agents.llm import LLM, ChatContext, ChatMessage, FunctionContext, LLMStream, ChatImage, ChatRole
from livekit.agents.log import logger
from livekit.agents.pipeline.agent_output import AgentOutput, SpeechSource, SynthesisHandle
from livekit.agents.pipeline.agent_playout import AgentPlayout
from livekit.agents.pipeline.human_input import HumanInput
from livekit.agents.pipeline.plotter import AssistantPlotter
...
def _default_before_llm_cb(
    agent: VoicePipelineAgent, chat_ctx: ChatContext, messages: List[Dict[str, Any]], ai_message_id: str
) -> LLMStream:
...
class VoicePipelineAgent(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        glow_messages = [{"role": msg.role, "content": msg.content, "id": msg.id} for msg in chat_ctx.messages]
        self.glow: GlowState = GlowState(messages=glow_messages, ai_message_id=str(uuid.uuid4()), user_message_id=str(uuid.uuid4()))
    ...
    async def _synthesize_answer_task(
        ...
        copied_glow_messages = copy.deepcopy(self.glow.messages)
        ...
        if playing_speech is not None and playing_speech.initialized:
            if (
                ...
                copied_glow_messages.append({"role": "assistant", "content": playing_speech.synthesis_handle.tts_forwarder.played_text, "id": self.glow.ai_message_id})

        copied_ctx.messages.append(
            ChatMessage.create(text=handle.user_question, role="user")
        )
        copied_glow_messages.append({"role": "user", "content": handle.user_question, "id": self.glow.user_message_id})

        llm_stream = self._opts.before_llm_cb(self, copied_ctx, copied_glow_messages, self.glow.ai_message_id)
        ...
        if not isinstance(llm_stream, LLMStream):
            llm_stream = _default_before_llm_cb(self, chat_ctx=copied_ctx, messages=copied_glow_messages, ai_message_id=self.glow.ai_message_id)
    ...
    async def _play_speech(self, speech_handle: SpeechHandle) -> None:
        ...
        def _commit_user_question_if_needed() -> None:
            ...
            self.glow.messages.append({"role": "user", "content": user_question, "id": self.glow.user_message_id})
            self.glow.user_message_id = str(uuid.uuid4())
        ...
        if speech_handle.add_to_chat_ctx and (
            ...
            self.glow.messages.append({"role": "assistant", "content": collected_text, "id": self.glow.ai_message_id})
            self.glow.ai_message_id = str(uuid.uuid4())
