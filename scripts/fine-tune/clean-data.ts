...
//   const model: TiktokenModel = 'gpt-4o-2024-08-06'
//   // TODO(docs): Just need enough for the LLM judge to understand the current conversation. Also,
//   // just need enough for the LLM judge to understand whether the message resulted in a positive or
//   // negative response from the user, which may occur multiple messages after the target assistant
//   // message. We include many subsequent user messages because a negative response could occur many
//   // messages after the assistant message.
//   const contextSize = 10 * ESTIMATED_TOKENS_PER_MESSAGE
...
//     const mainSystemPrompt = `You are an AI assistant whose goal is to classify whether the user responded negatively in response to the target message sent by the assistant, which was wrapped in <TARGET_MESSAGE> delimiters in an earlier system prompt. Negative responses can manifest as anger, disappointment, annoyance, dissatisfaction, or similar sentiments aimed at the assistant.
...
// 4. Assign values to `isNegativeResponse`